...
while True:
	mode = input("> Type\n> '0' or 'show' to show the plot,\n> '1' or 'point' to add points,\n> '2' or 'polygon' to add polygons,\n> '3' or 'polyline' to add polylines,\n> '4' or 'circle' to add circles,\n> '-1' or 'clear' to empty the plot\n> ")
	...
	if mode == '-1' or mode == 'clear':
		...
		for i in range(n):
			x, y = [int(v) for v in input('> x y: ').split(' ')]
			updates['point']['X'].append(x)
			updates['point']['Y'].append(y)

	elif mode == '2' or mode == 'polygon':
	...
	elif mode == '4' or mode == 'circle':
		n = int(input('> # of circles: '))
		for i in range(n):
			x, y, r = [int(v) for v in input('> x y r: ').split(' ')]
